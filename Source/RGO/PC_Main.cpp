...
void APC_Main::SetupInputComponent()
{
    ...
    if (auto EnhancedInputComponent = CastChecked<UEnhancedInputComponent>(InputComponent))
    {
        ...
        EnhancedInputComponent->BindAction(PauseInputAction, ETriggerEvent::Started, this, &APC_Main::PauseInput);
    }
}
...
void APC_Main::GameHasEnded(AActor* EndGameFocus, bool bIsWinner)
{
    if (CharacterLogicRef)
        CharacterLogicRef->HardDeath();
...
bool APC_Main::SetPause(bool bPause, FCanUnpause CanUnpauseDelegate)
{
    auto Result = Super::SetPause(bPause);
    if (!Result)
        return false;

    OnTogglePauseWidget.Broadcast(IsPaused());
...
void APC_Main::SetReferences()
{
    ControlledPawn        = nullptr;
    SpringArmComponentRef = nullptr;
        ...
        if (auto GameInstance = GetGameInstance<UGameInstance_Main>())
        {
            ...
            if (CharacterLogicRef)
            {
                ...
                if (auto World = GetWorld())
                {
                    ...
                    {
                        CharacterLogicRef->OnDeath.AddDynamic(GameMode, &AGM_Main::PlayerDead);
                    }
                }
            }
        }
...
void APC_Main::AddMappingContext()
{
            ...
            ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(GetLocalPlayer()))
    {
        ...
        EnhancedInputLocalPlayerSubsystem->AddMappingContext(UI_InputMappingContext, 0);
    }
}

void APC_Main::RemoveMappingContext()
{
    if (auto EnhancedInputLocalPlayerSubsystem =
            ULocalPlayer::GetSubsystem<UEnhancedInputLocalPlayerSubsystem>(GetLocalPlayer()))
    {
        EnhancedInputLocalPlayerSubsystem->RemoveMappingContext(InputMappingContext);
        EnhancedInputLocalPlayerSubsystem->RemoveMappingContext(UI_InputMappingContext);
    }
}
...
void APC_Main::PauseInput(const FInputActionValue& Value)
{
    SetPause(!IsPaused());
}
